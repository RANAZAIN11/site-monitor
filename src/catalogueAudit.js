...
export async function auditCatalogue(storeUrl) {
  ...
  const issues = []; // { sev, category, title, handle, url, problem, fix }

  const pushIssue = (sev, category, title, handle, problem, fix) => {
    issues.push({
      sev,
      category,
      title,
  ...
  for (const p of products) {
    ...
    if (/(^|-)copy(-|$)/.test(handle) || /copy-of/.test(handle)) {
      pushIssue(
        ...
        title,
        handle,
        `Handle contains "copy" (likely an accidental duplicate product).`,
        `In Shopify admin, check if this duplicates the real "${title}". Delete the copy (or fix its handle) and add a URL redirect from /products/${handle}.`
      );
      continue;
    }
    ...
    if (numMatch && handleMap.has(numMatch[1])) {
      pushIssue(
        ...
        title,
        handle,
        `Duplicate of "/products/${numMatch[1]}" (Shopify auto-added "-${numMatch[2]}").`,
        `Confirm the duplicate in admin. Keep one, delete the other, and redirect /products/${handle} to the correct one.`
      );
      continue;
    }
    ...
    if (images.length === 0) {
      pushIssue(
        ...
        title,
        handle,
        `Product has no images (page will look blank).`,
        `Add product images in Shopify admin > Products > "${title}".`
      );
    }
    ...
    if (allZero) {
      pushIssue(
        ...
        title,
        handle,
        `Price is 0 / missing on all variants.`,
        `Set the correct wholesale price in admin > Products > "${title}".`
      );
    }

    // 4b) some variants priced, some not (partial pricing — mixed sizes/colors unsellable)
    if (!allZero && variants.length > 1) {
      ...
      if (zeroCount > 0) {
        pushIssue(
          ...
          title,
          handle,
          `${zeroCount} of ${variants.length} variant(s) have price 0/missing while the rest are priced.`,
          `Check every size/colour variant's price in admin > Products > "${title}" — some options are currently unsellable.`
        );
      }
    }

    // 4c) compare-at price error (fake or negative discount)
    for (const v of variants) {
      ...
      if (isFinite(compareAt) && compareAt > 0 && isFinite(price) && compareAt <= price) {
        pushIssue(
          ...
          title,
          handle,
          `Variant "${v.title}" has a compare-at price (Rs ${v.compare_at_price}) that is <= the selling price (Rs ${v.price}) — shows a fake/zero/negative discount badge on the storefront.`,
          `In admin > Products > "${title}", either clear the compare-at price for this variant or set it higher than the selling price.`
        );
        break; // one mention per product is enough, avoid noisy repeats
      }
    }

    // 4d) stock check — every variant unavailable
    ...
    if (allUnavailable) {
      pushIssue(
        ...
        title,
        handle,
        `All variants are out of stock / unavailable for purchase.`,
        `Restock inventory in admin, or if it's discontinued, unpublish/archive the product instead of leaving a dead, unbuyable listing live.`
      );
    }
    ...
    if (/\b(test|untitled|dummy|asdf|sample product)\b/i.test(title)) {
      pushIssue(
        ...
        title,
        handle,
        `Title looks like a test/placeholder product.`,
        `Verify it is a real product; if not, delete it.`
      );
    }
    ...
    if (titleTokens.length > 0 && !handleHasToken && !/copy|\d+$/.test(handle)) {
      pushIssue(
        ...
        title,
        handle,
        `URL "/products/${handle}" doesn't match the product name "${title}" (maybe renamed).`,
        `If renamed, update the handle to "${slug}" and add a redirect from the old URL.`
      );
    }

    // 7) description missing / too short / placeholder text
    ...
    if (wordCount < 15) {
      pushIssue(
        ...
        title,
        handle,
        `Description is missing or too short (${wordCount} word(s)).`,
        `Add a proper description in admin > Products > "${title}" (fabric, fit, care instructions, sizing notes, etc.).`
      );
      ...
      pushIssue(
        ...
        title,
        handle,
        `Description contains placeholder/dummy text.`,
        `Replace the placeholder text with the real product description in admin > Products > "${title}".`
      );
  ...
  // 8) duplicate description copy-pasted across different products
    ...
    if (group.length > 1) {
      ...
      for (const dupe of dupes) {
        pushIssue(
          ...
          dupe.title,
          dupe.handle,
          `Description is identical/copy-pasted from "${original.title}" (/products/${original.handle}).`,
          `Write a unique description for "${dupe.title}" — duplicate content hurts SEO and looks lazy to buyers.`
        );
      }
    }
  }
  ...
  return { issueCount: issues.length, totalProducts: total, issues, text };
}
